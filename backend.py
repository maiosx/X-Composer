#!/usr/bin/env python3
"""x.composer backend — job-based posting helper for the Omarchy plugin.
...
USER_AGENT = "x.composer-omarchy/1.0"
...
def ensure_runtime() -> Path:
    ...
    rt = Path(base) / "x.composer"
    if rt.exists():
        _refuse_bad_path(rt, True, "$XDG_RUNTIME_DIR/x.composer")
    else:
        with contextlib.suppress(FileExistsError):
            rt.mkdir(mode=0o700)
        os.chmod(rt, 0o700)
    jobs = rt / "jobs"
    if jobs.exists():
        _refuse_bad_path(jobs, True, "$XDG_RUNTIME_DIR/x.composer/jobs")
    else:
        with contextlib.suppress(FileExistsError):
